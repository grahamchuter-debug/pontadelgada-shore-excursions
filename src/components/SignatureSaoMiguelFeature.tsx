...
export function SignatureSaoMiguelFeature({ embedded = false }: { embedded?: boolean }) {
  const image = subjectImages["sete-cidades"];
  const comingSoon = signatureSaoMiguelExperience.comingSoon;
    ...
    <div className="card-signature grid gap-0 overflow-hidden lg:grid-cols-2">
      ...
      <div className="flex flex-col justify-center p-8 sm:p-10 lg:p-12">
        ...
        <h2 className="font-display text-3xl font-semibold text-gray-900 sm:text-4xl mt-1">
          {signatureSaoMiguelExperience.title}
        </h2>
        <p className="mt-4 text-base leading-relaxed text-gray-600 italic">
          &ldquo;{signatureSaoMiguelExperience.tagline}&rdquo;
        </p>
        <p className="mt-4 text-sm leading-relaxed text-gray-700">
          {comingSoon
            ...
            : "The experience we would honestly recommend to a first-time cruise passenger who wants the very best of São Miguel in a single port day."}
        </p>
        ...
        {comingSoon ? (
          <div className="placeholder-widget mt-6 text-left">
            ...
            <p className="mt-2 text-sm text-gray-600">
              Sete Cidades&apos; twin lakes, a carefully selected viewpoint route, photography stops and
              a local lunch — proposed with a maximum of eight guests and cruise-first timing. Register
              your interest to hear when the concept is ready.
            </p>
          </div>
        ) : (
          <div className="mt-6 grid gap-2 sm:grid-cols-2">
            {signatureSaoMiguelExperience.benefits.slice(0, 4).map((b) => (
        ...
        <div className="mt-8 flex flex-wrap gap-3">
          <Link href={SIGNATURE_EXPERIENCE_PATH} className="btn-accent">
            {comingSoon ? "Learn more →" : `Discover ${signatureSaoMiguelExperience.title} →`}
          </Link>
          {comingSoon && (
