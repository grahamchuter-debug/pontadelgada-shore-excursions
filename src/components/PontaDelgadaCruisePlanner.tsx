...
export function PontaDelgadaCruisePlanner() {
  const [arrivalTime, setArrivalTime] = useState("07:30");
  const [departureTime, setDepartureTime] = useState("17:00");
  const [adults, setAdults] = useState("2");
  const [children, setChildren] = useState("0");
  const [interests, setInterests] = useState<string[]>(["ponta-delgada", "sete-cidades"]);
  ...
  function downloadPdf() {
    ...
    doc.save("ponta-delgada-cruise-plan.pdf");
  }
  ...
  return (
    <div className="card-feature">
      ...
      <div className="mt-6 flex flex-wrap gap-3">
        <button type="button" onClick={generate} className="btn-primary">Build my São Miguel plan</button>
      ...
      {plan && (
        <div className="mt-8 space-y-8">
          ...
          <p className="text-xs text-gray-500">
            ...
            minute return buffer. Crater-rim cloud and road time to Sete Cidades, Lagoa do Fogo or
            Furnas can lengthen journey times.
          </p>
        </div>
      )}
    </div>
