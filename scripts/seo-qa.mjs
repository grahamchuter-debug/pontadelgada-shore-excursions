#!/usr/bin/env node
/**
 * Post-build SEO QA checks for Ponta Delgada Shore Excursions static export.
 */
...
if (!existsSync(OUT)) {
  ...
  // /cruise-port-guide and /ship-schedules(/ponta-delgada) are legacy alias routes,
  // 301-redirected at the edge via public/_redirects, and intentionally noindexed
  // in favour of their canonical replacements (/ponta-delgada-cruise-port,
  // /ponta-delgada-cruise-ship-schedule).
  const NOINDEX_ALIAS_PATTERNS = ["/privacy/", "/terms/", "/cruise-port-guide/", "/ship-schedules/"];
  const indexableMustNotHaveNoindex = htmlFiles.filter(
    (f) => !NOINDEX_ALIAS_PATTERNS.some((p) => f.includes(p)),
  );
...
// 8. _redirects (Workers Static Assets V1.0)
// Host-level www/http → apex belongs in Cloudflare Redirect Rules + Always Use HTTPS.
// Absolute host rules in _redirects are rejected by Workers (error 100324).
...
if (existsSync(redirects)) {
  ...
  if (text.includes("https://www.")) {
    warnings.push(
      "_redirects contains absolute www host rules — Workers Static Assets require relative path redirects only; use Cloudflare Redirect Rules for www→apex",
    );
  }
} else {
...
console.log("  • WWW→non-WWW — Cloudflare Redirect Rules (Workers Static Assets; not _redirects host rules)");
console.log("  • Path aliases — relative 301s in public/_redirects");
